...
class EditCardActivity
    ...
    companion object {
        const val EXTRA_CREATE_NEW_CARD = "com.izzdarki.wallet.edit_card_activity.create_new_card"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (isCreateNewCardIntent) {
            ...
            /* explanation (i don't know what this means or whether this is up to date, but i also don't want to know it)
        ...
        cardNameInputEditText.onFocusChangeListener = OnFocusChangeListener { _: View?, hasFocus: Boolean ->
            if (!hasFocus) {
                ...
                // Mahler easter egg
                if (isCreateNewCardIntent && cardName == getString(R.string.mahler_is_cool))
                    makeMahlerCard()
            }
        }
        if (isCreateNewCardIntent) {
            // ! Keyboard doesn't show up. In EditPasswordActivity the exact same code works an keyboard shows up
            // When creating a new card start with name selected
            cardNameInputEditText.requestFocus()
            cardNameInputEditText.selectAll()
        }
