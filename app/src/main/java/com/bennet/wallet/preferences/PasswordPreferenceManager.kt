...
object PasswordPreferenceManager : CardOrPasswordPreferenceManager {
    private const val PASSWORDS_PREFERENCES_NAME = "passwords"
    ...
    /**
     * @return Password color or default color [R.color.password_default_color] if not found in preferences
     */
    override fun readColor(context: Context, ID: Int): Int {
        return getPreferences(context).getInt(
            getKey(ID, PREFERENCE_PASSWORD_COLOR),
            context.resources.getColor(R.color.password_default_color))
    }
    ...
    override fun readCustomSortingWithGrouping(context: Context): Map<String, List<String>> {
        return AppUtility.CustomSortingWithGrouping.fromString(
            CardPreferenceManager.getPreferences(context).getString(PREFERENCE_CUSTOM_SORTING_WITH_GROUPING, "")!!
        )
    }
